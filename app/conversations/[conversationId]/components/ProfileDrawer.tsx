...
const ProfileDrawer: React.FC<ProfileDrawerProps> = ({
  ...
  const otherUserName = otherUser.name || 'Anonymous'; // Default value if name is null

  const joinedDate = useMemo(() => format(new Date(otherUser.createdAt), 'PP'), [otherUser.createdAt]);
  const title = useMemo(() => data.name || otherUserName, [data.name, otherUserName]);
  const { members } = useActiveList();
  const isActive = members.includes(otherUser.email || ''); // Default value if email is null
  ...
  }, [data.isGroup, isActive, data.users.length]);
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          <div className="w-full pb-5 pt-5 sm:px-0 sm:pt-0">
                            <dl className="space-y-8 px-4 sm:space-y-6 sm:px-6">
                              {data.isGroup && (
                                <div>
                                  <dt className="text-sm font-medium text-gray-500 sm:w-40 sm:flex-shrink-0">Names</dt>
                              ...
                              {!data.isGroup && (
                                <div>
                                  ...
                                  <dd className="mt-1 text-sm text-gray-900 sm:col-span-2">{otherUserName}</dd>
                                </div>
                              )}
                              {!data.isGroup && (
                                <>
